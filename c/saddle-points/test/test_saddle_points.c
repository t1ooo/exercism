...
static void print_points(char* name, size_t count, saddle_point_t a[]) {
   for(size_t i=0; i<count; i++) {
      saddle_point_t t = a[i];
      printf("%s: %ld %ld\n", name, t.row, t.column);
   }
}
...
static void test_single_saddle_point(void)
{
   ...
   // print_points("expected", expected_count, expected_points);
   // print_points("actual", actual->count, actual->points);
...
static void test_lack_of_saddle_points_when_there_are_none(void)
{
   ...
   print_points("expected", expected_count, NULL);
   print_points("actual", actual->count, actual->points);
