...
#[test]
// #[ignore]
fn basics_push_pop_at_back_2() {
    let mut list: LinkedList<i32> = (0..10).collect();
    assert_eq!(list.len(), 10);
...
fn iter() {
    ...
    assert_eq!(10, list.iter().count());
    assert_eq!(Some(&0), list.iter().next());
    assert_eq!(Some(&9), list.iter().last());
...
fn collect_to_vec() {
    ...
    for (num, &entered_num) in (0..10).zip(vec.iter()) {
        assert_eq!(num, entered_num);
    }
}
