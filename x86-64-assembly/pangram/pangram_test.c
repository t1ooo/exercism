...
void test_one_char_case(void) {
    //TEST_IGNORE();
    TEST_ASSERT_FALSE(is_pangram("a"));
}

void test_perfect_lower_case(void) {
    //TEST_IGNORE();
    int result = is_pangram("abcdefghijklmnopqrstuvwxyz");
    //printf("test_perfect_lower_case: %d\n", result);
...
void test_only_lower_case(void) {
    ...
    int result = is_pangram("the quick brown fox jumps over the lazy dog");
    printf("test_only_lower_case: %d\n", result);
    TEST_ASSERT_TRUE(result);
}
